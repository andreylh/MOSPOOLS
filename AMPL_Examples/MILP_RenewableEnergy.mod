...
var totalChargeAndDischarge >=0;
var obj1Cost;
var obj2CostBatteries;
var obj3MaxLoad, >=0;
...
calcTotalCharge: 
        totalCharge = sum{i in I} sum{v in PEV} sum{c in C} (yCharge[v,c,i]*pevChargeRate[v,c]/100*pevPower[v]);

calcTotalDischarge: 
        totalDischarge = sum{i in I} sum{v in PEV} sum{c in C} (yDischarge[v,c,i]*pevDischargeRate[v,c]/100*pevPower[v]);

calcTotalChargeDischare:
	totalChargeAndDischarge = totalCharge+totalDischarge;

obj1TotalCostWithGrid: 
	obj1Cost = (sum{i in I} (energyBuying[i]*priceBuying[i] - energySelling[i]*priceSelling[i]) )/100;

obj2TotalCostBatteries: 
	obj2CostBatteries = (sum{i in I} sum{v in PEV} sum{c in C} (yDischarge[v,c,i]*pevDischargePrice[v,c]*pevDischargeRate[v,c]/100*pevPower[v] + yCharge[v,c,i]*pevChargePrice[v,c]*pevChargeRate[v,c]/100*pevPower[v]) )/100;

obj3CalcMaxLoad1{i in I}: 
	obj3MaxLoad >= energySelling[i] + energyBuying[i]; 

minimize totalPrice:  1*obj1Cost + 1*obj2CostBatteries + 1*obj3MaxLoad;
